...
def ManhattanDistance(index1, index2):  # The function that calculates Manhattan distance
    columDistance = abs(index1 % 3 - index2 % 3)  # Distance in rows between the indexes
    rowsDistance = abs(int(index1 / 3) - int(index2 / 3))  # Distance in columns between the indexes
    return columDistance + rowsDistance
...
def hdistance1(s):  # the heuristic function. returns the Manhattan distance calculation
    myBoard = s[0] # the board in the current state
    goalBoard = list(range(len(s[0])))  # [0,1,2,3...]
    s = 0
    for i in range(0, len(goalBoard)):
        s += ManhattanDistance(i, goalBoard.index(myBoard[i]))  # summing up the Manhattan distances of all elements
        return s


def hdistance2(s):  # the heuristic function. returns the number of elements which are not in their place
    myBoard = s[0]  # the board in the current state
    goalBoard = list(range(len(s[0])))  # [0,1,2,3...]
    s = 0
    for i in range(0, len(goalBoard)):
        if goalBoard[i] != myBoard[i]:  # counting the number of elements that are out of place
            s += 1
    return s
